...
async def linkedin_connect(request: Request):
    ...
    redirect_uri = (
        ...
        or "http://localhost:8000/api/linkedin/callback"
    )
...
async def linkedin_callback(request: Request):
    ...
    redirect_uri = os.getenv("REDIRECT_URI") or os.getenv("LINKEDIN_REDIRECT_URI") or "http://localhost:8000/api/linkedin/callback"
...
async def auth_register(response: Response, body: Dict = Body(...)):
    ...
    # NEW: set session cookie for token
    try:
        response.set_cookie(
            key="auth_token",
            value=token,
...
async def auth_logout(request: Request, response: Response):
    ...
    # also clear cookie-based session (match set_cookie path)
    try:
        cookie_tok = request.cookies.get("auth_token")
        if cookie_tok:
